...
f_ = ti.field(shape = (n_curling_stones), dtype = real) # 平均作用力
theta = ti.field(shape = (n_curling_stones), dtype = real) # 初始角度
w = ti.field(shape = (steps, n_curling_stones), dtype = real)
...
def time_integrate(t: ti.i32):
    for i in range(n_curling_stones):
        ...
        theta[i] -= dt * w[t - 1, i]
        v_total = v[t - 1, i].norm() - miu * g * dt
        if i == 0:
            print(v_total)

        v[t, i][0] = v_total * ti.cos(theta[i]) # velocity along x-axis
        v[t, i][1] = v_total * ti.sin(theta[i]) # velocity along y-axis
        v[t, i] = v[t, i] + impulse[t, i]
        w[t, i] = v_total * f_[i] / r_p # Angular velocity


        x[t, i][0] = x[t - 1, i][0] + dt * v[t, i][0]
        x[t, i][1] = x[t - 1, i][1] + dt * v[t, i][0] * theta[i]
        x[t, i] = x[t, i] + x_inc[t, i]
...
def initial():
    for i in range(n_curling_stones):
        v_total0 = v[0, i].norm()
        if v_total0 != 0:
            f_[i] = w[0, i] * r_p / v_total0
        else:
            f_[i] = 0
        theta[i] = ti.atan2(v[0, i][1],v[0, i][0])
